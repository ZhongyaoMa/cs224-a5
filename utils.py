...
def pad_sents_char(sents, char_pad_token):
    ...
    max_sents_length = max([len(s) for s in sents])
    ...
    for i in range(len(sents_padded)):
        for j in range(len(sents_padded[i])):
            ...
            sents_padded[i][j] += [char_pad_token] * (max_word_length - len(sents_padded[i][j]))
        sents_padded[i] += [[char_pad_token] * max_word_length] * (max_sents_length - len(sents_padded[i])) 
...
def pad_sents(sents, pad_token):
    ...
    for i in range(len(sents)):
        sents_padded.append(sents[i] + [pad_token] * (longest_len - len(sents[i])))
