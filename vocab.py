...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.char2id['{']] + [self.char2id[c] if c in self.char2id else self.char2id['<unk>'] for c in w] + [self.char2id['}']] for w in s] for s in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_padded = pad_sents_char(self.words2charindices(sents), self.char2id['<pad>'])  # batch_size * max_sents_len * max_word_len
